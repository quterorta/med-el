    ...
    <section class="slider">
        <div class="main-page-slider">
            @foreach($slider->images as $image)
                <img src="{{ Storage::url($image->image_path) }}" alt="" class="main-page-slider-image">
            @endforeach
        </div>
    </section>

    <section id="populare" class="popular-products-section block-plr-5 block-ptb-5">
        <p class="popular-products-section-header block_header">Produse populare</p>
        @if(count($popularProducts) > 0)
        @include('blocks.products.popular-products-slider')
        @endif
    </section>

    <section id="noi" class="new-products-section block-plr-5 block-ptb-5">
        <p class="new-products-section-header block_header">Produse Noi</p>
        @if(count($newProducts) > 0)
            @include('blocks.products.new-products-slider')
        @endif
    </section>
